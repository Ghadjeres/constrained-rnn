...
class ConstraintModel_Alternative(nn.Module):
    ...
    def forward(self, x: Variable):
        ...
        # constraints:
        hidden = (Variable(torch.zeros(self.num_layers, batch_size, self.num_lstm_constraints_units).cuda()),
                  Variable(torch.zeros(self.num_layers, batch_size, self.num_lstm_constraints_units).cuda()))
        ...
        # generation:
        hidden = (Variable(torch.zeros(self.num_layers, batch_size, self.num_lstm_generation_units).cuda()),
                  Variable(torch.zeros(self.num_layers, batch_size, self.num_lstm_generation_units).cuda()))

        offset_seq = torch.cat([Variable(torch.zeros(1, batch_size, self.num_features).cuda()), seq[:seq_length - 1, :,:]], 0)
        ...
        weights = [self.linear_2(time_slice) for time_slice in output_gen]
...
if __name__ == '__main__':
    ...
    constraint_model = ConstraintModel_Alternative(num_features=num_features, num_layers=2)
